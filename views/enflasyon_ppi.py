...
BLS'in Üretici Fiyat Endeksi (PPI) verilerini gösterir: başlık göstergeleri
(Nihai Talep, Çekirdek ÜFE, Enerji, Gıda, Hizmetler...), hizmet alt kategorileri
ve Ara Talep (Intermediate Demand) — üretim zincirinin farklı aşamalarındaki
fiyat baskısını gösteren, genelde nihai talep enflasyonundan önce hareket eden
öncü göstergeler.

PPI bir SEVİYE (istihdam sayısı) değil bir ENDEKS (Kasım 2009=100 taban yıllı)
olduğundan, mutlak değer karşılaştırması yerine odak HER ZAMAN % değişim
(aylık ve yıllık üretici enflasyonu oranı) üzerindedir.
"""
...
from src.ppi_catalog import get_categories, get_by_category
...
def latest_index_and_changes(df: pd.DataFrame):
    """PPI endeksi için: son değer, aylık % değişim, yıllık % değişim."""
...
# ---------------------------------------------------------------- ana başlık
st.title("💰 ÜFE (PPI) — Genel Bakış")
st.caption("Kaynak: U.S. Bureau of Labor Statistics — Producer Price Index (PPI)")
...
# ---------------------------------------------------------------- özet kartlar
st.subheader(f"{category} — Güncel ÜFE Oranları")
    ...
    with col:
        if latest is not None and yoy_pct is not None:
            st.metric(
                ...
                help=f"Endeks değeri: {latest['value']:.2f} (Kasım 2009=100)",
            )
...
# ---------------------------------------------------------------- üfe bar grafiği (ay seçilebilir)
st.subheader("ÜFE Oranları Karşılaştırması (Ay Seçilebilir)")
st.caption(
...
bar_metric = st.radio(
    ...
    horizontal=True,
    key=f"ppi_bar_metric_{category}",
)
...
if bar_lines_all.empty:
    ...
    if month_df.empty:
        ...
        st.caption("Not: ÜFE'de kırmızı=artış, yeşil=düşüş renklendirmesi kullanılmıştır (istihdam grafiklerinin tersi).")
...
view_mode = st.radio(
    "Gösterim",
    ["Yıllık % Değişim (ÜFE Oranı)", "Aylık % Değişim", "Ham Endeks Değeri"],
    horizontal=True,
    key="ppi_ts_view_mode",
)
...
if selected_series:
    if view_mode == "Ham Endeks Değeri":
        ...
        ts_start, ts_end = report_utils.date_range_slider(selected_dfs, key=f"ppi_ts_daterange_{category}")
        fig = go.Figure()
        ...
        report_utils.render_pct_change_chart(
            lines, key_prefix=f"ppi_ts_{category}_{pct_type}",
...
if table_view == "Tekli kalem (detaylı)":
    ...
    if compact.empty:
        ...
        # PPI'da "Değer" bir endeks (2-3 ondalık anlamlı), Aylık/Yıllık Değişim (index puan
        ...
        st.download_button(
            ...
            file_name=f"{single_sid}_ppi_aylik_rapor.csv",
            mime="text/csv",
            key="download_ppi_compact",
        )
else:
    ...
    if value_type_label == "Aylık % Değişim":
        # build_wide_report_table'ın "change" modu MUTLAK fark verir; PPI için
    ...
    if wide.empty:
        ...
        st.download_button(
            ...
            file_name=f"ppi_{category.lower()}_genis_tablo.csv",
            mime="text/csv",
            key="download_ppi_wide",
        )
...
with st.expander("📄 Ham veriyi görüntüle / indir (tüm kategori)"):
    ...
    if combined:
        ...
        st.download_button(
            ...
            file_name=f"ppi_{category.lower()}_data.csv",
            mime="text/csv",
        )
